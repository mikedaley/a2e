...
void DiskII::reset()
{
  ...
  for (auto &drive : drives_)
  {
    ...
    drive.last_returned_nibble = 0;
    drive.last_returned_track = -1;
  }
...
uint8_t DiskII::readNibble()
{
  ...
  // Use quarter-track position directly for more accurate emulation
  int quarter_track = drive.quarter_track;
  int track_size = drive.disk->getNibbleTrackSizeAtQuarterTrack(quarter_track);
  ...
  // Simplified timing for DSK format:
  // Just return the nibble at the current position and advance.
  // The disk data is already nibblized with proper GCR encoding,
  // so every byte has bit 7 set naturally.

  int current_position = drive.nibble_position;
  uint8_t nibble = drive.disk->getNibbleAtQuarterTrack(quarter_track, current_position);

  // Advance to next position for next read
  drive.nibble_position = (current_position + 1) % track_size;

  // Return the nibble as-is (it already has bit 7 set from GCR encoding)
  data_latch_ = nibble;
...
bool DiskII::insertDisk(int drive_num, std::unique_ptr<DiskImage> image)
{
  ...
  drives_[drive_num].last_returned_nibble = 0;
  drives_[drive_num].last_returned_track = -1;
...
int DiskII::getQuarterTrack(int drive_num) const
{
  if (drive_num < 0 || drive_num > 1)
  {
    return 0;
  ...
  return drives_[drive_num].quarter_track;
}

float DiskII::getTrackPosition(int drive_num) const
{
  if (drive_num < 0 || drive_num > 1)
  {
    return 0.0f;
  ...
  // Convert quarter-track to float: 69 -> 17.25, 70 -> 17.50, etc.
  int qt = drives_[drive_num].quarter_track;
  return static_cast<float>(qt) / 4.0f;
}

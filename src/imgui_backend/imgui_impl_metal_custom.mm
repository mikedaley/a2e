...
// Global sampler mode: true = linear filtering, false = nearest neighbor
static bool g_ImGui_Metal_UseLinearSampler = false;

void ImGui_ImplMetal_SetSamplerLinear(bool use_linear)
{
    g_ImGui_Metal_UseLinearSampler = use_linear;
}

bool ImGui_ImplMetal_GetSamplerLinear()
{
    return g_ImGui_Metal_UseLinearSampler;
}
    ...
    for (const ImDrawList* draw_list : draw_data->CmdLists)
    {
        ...
        for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                // Bind texture and sampler, Draw
                if (ImTextureID tex_id = pcmd->GetTexID())
                    [commandEncoder setFragmentTexture:(__bridge id<MTLTexture>)(void*)(intptr_t)(tex_id) atIndex:0];
                
                // Set sampler based on global filtering mode
                id<MTLSamplerState> sampler = g_ImGui_Metal_UseLinearSampler ? ctx.linearSampler : ctx.nearestSampler;
...
bool ImGui_ImplMetal_CreateDeviceObjects(id<MTLDevice> device)
{
    ...
    // Linear filtering sampler (smooth)
    samplerDescriptor.minFilter = MTLSamplerMinMagFilterLinear;
    samplerDescriptor.magFilter = MTLSamplerMinMagFilterLinear;
    samplerDescriptor.mipFilter = MTLSamplerMipFilterLinear;
    bd->SharedMetalContext.linearSampler = [device newSamplerStateWithDescriptor:samplerDescriptor];
    
    // Nearest neighbor sampler (sharp pixels)
    samplerDescriptor.minFilter = MTLSamplerMinMagFilterNearest;
    samplerDescriptor.magFilter = MTLSamplerMinMagFilterNearest;
    samplerDescriptor.mipFilter = MTLSamplerMipFilterNearest;
    bd->SharedMetalContext.nearestSampler = [device newSamplerStateWithDescriptor:samplerDescriptor];

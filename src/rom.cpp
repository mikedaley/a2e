...
bool ROM::loadDiskIIROM(const std::string &filepath)
{
  ...
  if (!file.is_open())
  {
    std::cerr << "Failed to load Disk II ROM: " << filepath << std::endl;
    return false;
  }

  // P5A ROM is 256 bytes, loads at $C600 (offset $500 in expansion_rom_)
  // $C600 - $C100 = $500 = 1280
  constexpr size_t P5A_SIZE = 256;
  constexpr size_t P5A_OFFSET = 0x500;

  file.read(reinterpret_cast<char *>(expansion_rom_.data() + P5A_OFFSET), P5A_SIZE);
  size_t bytes_read = file.gcount();
  file.close();

  if (bytes_read != P5A_SIZE)
  {
    std::cerr << "Disk II ROM size mismatch (expected 256, read "
              << bytes_read << ")" << std::endl;
    return false;
  }
